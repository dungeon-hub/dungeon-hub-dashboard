...
import {
	formatLinkedUserCount,
	GlobalStatsComponent,
} from "./global-stats.component";

describe("formatLinkedUserCount", () => {
	it("groups linked user counts without converting them to a number", () => {
		expect(formatLinkedUserCount("123456789012345678")).toBe(
...
describe("GlobalStatsComponent", () => {
	...
	it("loads linked users and displays every global statistic", () => {
		...
		expect(text).toContain(
			"Users flagged for illegitimate or harmful activity",
		);
	...
	it("formats trend text and class thresholds without extra labels", () => {
		...
		expect(
			component.getTrendValue(
				{ stat, usesPrecedingWindows: true },
				"last30Days",
			),
		).toBe("-39.9%");
		expect(
	...
	it("shows errors and retries through the rendered button", () => {
		...
		getGlobalStats
			...
			.mockReturnValueOnce(of({ linkedUsers: "42" }));
		const fixture = TestBed.createComponent(GlobalStatsComponent);
