...
export class DiscordGuildService {
	...
	getAllGuilds(): DiscordGuild[] {
		...
		return (
			claims["discord-guilds"] || claims.guilds || claims.discord_guilds || []
		);
	}
	...
	getDisplayName(guild: DiscordGuild | string): string {
		let displayName = typeof guild === "string" ? guild : guild.name;
		...
		displayName = displayName.replace(
			...
			(_match, name) => {
				...
				// Try exact match
				const emoji =
					this.emojiNameMap.get(name) ||
					this.emojiNameMap.get(name.replace(/_/g, "-"));
	...
	getIconUrl(guild: DiscordGuild): string {
		...
		const extension = guild.icon.startsWith("a_")
			? "webp?animated=true"
			: "png";
