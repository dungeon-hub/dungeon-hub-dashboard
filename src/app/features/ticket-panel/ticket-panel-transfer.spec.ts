...
describe("ticket panel transfer", () => {
	it("clones all settings without modifying the previous panel", () => {
		...
		(clone.permissions as Record<string, Record<string, string>>)["Everyone"][
			"Denied"
		] = "0";
	...
	it("exports and imports a detached panel without modifying the previous panel", () => {
		const original = panel();
		const snapshot = structuredClone(original);
		const imported = parseTicketPanelExport(
			JSON.stringify(exportTicketPanel(original)),
		);
	...
	it("exports all panels as a detached backup without mutating source data", () => {
		...
		(
			backup.panels[0].panel.permissions as Record<
				string,
	...
	it("parses exported backups and raw arrays as multiple detached imports", () => {
		...
		const backupImports = parseTicketPanelExports(
			serializeTicketPanels([first, second]),
		);
		const arrayImports = parseTicketPanelExports(
			JSON.stringify([exportTicketPanel(first), exportTicketPanel(second)]),
		);
		(
			backupImports[0].panel.permissions as Record<
				string,
				Record<string, string>
			>
		)["Everyone"]["Denied"] = "0";

		expect(backupImports.map((item) => item.panel.name)).toEqual([
			"support",
			"billing",
		]);
	...
	it.each([
		...
		[
			...
			JSON.stringify([exportTicketPanel(panel()), { version: 1 }]),
		],
	])("rejects invalid multi-panel exports: %s", (_label, contents) => {
		expect(() => parseTicketPanelExports(contents)).toThrow(
	...
	it("serializes an all-panel backup as reusable pretty JSON", () => {
		...
		expect(
			parseTicketPanelExport(JSON.stringify(parsed.panels[0])).panel.name,
		).toBe("support");
		expect(
			parseTicketPanelExport(JSON.stringify(parsed.panels[1])).panel.name,
		).toBe("billing");
	});
	...
	it("serializes pretty JSON that can be used by file and clipboard import", () => {
		...
		expect(parseTicketPanelExport(serialized).panel.ticketMessage).toBe(
			'{"content":"Help"}',
		);
	});
	...
	it("finds an overwrite conflict by exact ID and name or by an existing name", () => {
		...
		expect(findImportConflict([existing], { id: "1", name: "support" })).toBe(
			existing,
		);
		expect(findImportConflict([existing], { id: "4", name: "support" })).toBe(
			existing,
		);
		expect(findImportConflict([existing], { name: "support" })).toBe(existing);
		expect(findImportConflict([existing], { name: " SUPPORT " })).toBe(
			existing,
		);
		expect(
			findImportConflict([existing], { id: "1", name: "other" }),
		).toBeUndefined();
	...
	it("builds a detached overwrite model and resets settings absent from the import", () => {
		...
		(update.permissions as Record<string, Record<string, string>>)["Everyone"][
			"Denied"
		] = "0";
	...
	it("rejects unsupported import files", () => {
		expect(() =>
			parseTicketPanelExport('{"version":999,"panel":{"name":"x"}}'),
	...
	it.each([
		...
		[
			"transcript target",
			(data: any) => (data.panel.closeTranscriptTarget = "Unknown"),
		],
		...
		[
			"form type",
			(data: any) =>
				(data.panel.formQuestions = [{ type: "Unknown", data: "{}" }]),
		],
		[
			"form data",
			(data: any) =>
				(data.panel.formQuestions = [{ type: "TextInput", data: 123 }]),
		],
		[
			"permission group",
			(data: any) => (data.panel.permissions = { Everyone: [] }),
		],
		[
			"permission bit set",
			(data: any) => (data.panel.permissions = { Everyone: { Denied: {} } }),
		],
	])(
		"rejects invalid %s values before they can reach the API",
		(_field, invalidate) => {
			const exported = exportTicketPanel(panel()) as any;
			invalidate(exported);
	...
	it("accepts every supported transcript target and form question type", () => {
		...
		for (const target of transcriptTargets) {
			for (const type of formTypes) {
				...
				expect(
					parseTicketPanelExport(JSON.stringify(exported)).panel,
				).toMatchObject({
