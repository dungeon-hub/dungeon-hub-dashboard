...
describe("TicketPanelListComponent transfers", () => {
	...
	beforeEach(async () => {
		...
		guildService.getGuildById.mockReturnValue({
			id: "server-1",
			name: "Dungeon Server",
		});
		...
		await TestBed.configureTestingModule({
			...
			providers: [
				provideRouter([]),
				{
					...
					useValue: { snapshot: { params: { serverId: "server-1" } } },
				},
	...
	it("recognizes an export-all backup copied from the clipboard as importable", async () => {
		...
		expect(
			instance.importCandidates.map(
				(candidate) => candidate.imported.panel.name,
			),
	...
	it("copies an all-panel backup to the clipboard without mutating current panels", async () => {
		...
		expect(clipboard.writeText).toHaveBeenCalledWith(
			serializeTicketPanels([existing, second]),
		);
	...
	it("offers overwrite when an import has the same internal name but a different source ID", async () => {
		...
		await instance.onFileDrop({
			...
			dataTransfer: {
				files: [
					{ text: () => Promise.resolve(serializeTicketPanel(imported)) },
				],
			},
		} as any);
		...
		await instance.onFileDrop({
			...
			dataTransfer: {
				files: [
					{
						text: () =>
							Promise.resolve(serializeTicketPanels([selected, skipped])),
					},
				],
			},
		} as any);
		...
		await instance.onFileDrop({
			...
			dataTransfer: {
				files: [
					{
						text: () =>
							Promise.resolve(serializeTicketPanels([existing, second])),
					},
				],
			},
		} as any);
	...
	it("requires at least one selected panel before continuing a backup import", async () => {
		...
		instance.importCandidates.forEach((candidate) => {
		...
		expect(instance.createError).toBe(
			"Select at least one ticket panel to import.",
		);
	...
	it("clones detached settings and rejects duplicate names case-insensitively", () => {
		...
		const copiedPermissions = instance.pendingPanel?.permissions;
		if (!copiedPermissions) throw new Error("Expected copied permissions");
		copiedPermissions["Everyone"]["Denied"] = "changed";
		instance.newPanel = {
			name: " SUPPORT ",
	...
	it.each([
		[
			"cloned",
			(instance: TicketPanelListComponent) => instance.openCopyModal(existing),
		],
		[
			"imported",
	...
	it("reports panel loading failures", () => {
		service.getAllTicketPanels.mockReturnValueOnce(
			throwError(() => new Error("offline")) as any,
		);
		const instance = component();
