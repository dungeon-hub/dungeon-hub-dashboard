...
export type StaticMessageType =
	StaticMessageCreationModel.StaticMessageTypeEnum;
export type StaticMessageObjectOption = { id: string; name: string };

type ObjectOptionType = Extract<
	StaticMessageType,
	"TicketPanel" | "ScoreLeaderboard" | "PriceMessage"
>;
...
export function supportsObjectIds(
	type: StaticMessageType,
): type is ObjectOptionType {
	return type in STATIC_MESSAGE_OBJECT_OPTION_TYPES;
}

export function getObjectOptionTypeLabel(
	type: StaticMessageType,
): string | null {
...
export function toTicketPanelOption(
	panel: TicketPanelModel,
): StaticMessageObjectOption {
	return { id: panel.id, name: panel.displayName || panel.name || panel.id };
}

export function toCarryTypeOption(
	carryType: CarryTypeModel,
): StaticMessageObjectOption {
	return {
		id: carryType.id,
		name: carryType.displayName || carryType.identifier || carryType.id,
	};
}

export function toCarryTierOption(
	carryTier: CarryTierModel,
): StaticMessageObjectOption {
	const carryTypeName =
		carryTier.carryType?.displayName || carryTier.carryType?.identifier;
	const carryTierName =
		carryTier.displayName || carryTier.identifier || carryTier.id;
	return {
		id: carryTier.id,
		name: carryTypeName ? `${carryTypeName} - ${carryTierName}` : carryTierName,
	};
}
