...
describe("getDiscordUserId", () => {
	...
	it("returns an empty string when the token or Discord id claim is not present", () => {
		...
		expect(
			getDiscordUserId(tokenWithPayload('{"sub":"not-a-discord-id"}')),
		).toBe("");
...
describe("ServerStatsComponent carry count", () => {
	it("requests and displays the new server stats endpoint for the current user", async () => {
		...
		await TestBed.configureTestingModule({
			...
			providers: [
				provideRouter([]),
				{
					...
					useValue: {
						paramMap: of(convertToParamMap({ serverId: "server-1" })),
					},
				},
				{
					provide: DiscordGuildService,
					useValue: { getGuildById: () => undefined },
				},
		...
		expect(
			fixture.nativeElement
				.querySelector(".grid")
				?.classList.contains("xl:grid-cols-5"),
		).toBe(true);
		expect(getServerStats).toHaveBeenCalledWith("server-1");
		expect((fixture.componentInstance as any).stats.yourCompletedCarries).toBe(
			1250,
		);
		...
		await TestBed.configureTestingModule({
			...
			providers: [
				...
				},
				{
					...
					useValue: {
						getServerStats: () => throwError(() => new Error("unavailable")),
					},
				},
			],
		}).compileComponents();
	...
	it("cancels an active request and clears stats when routing to a missing server ID", async () => {
		...
		expect((fixture.componentInstance as any).error).toBe(
			"Your server could not be identified.",
		);
		expect(fixture.nativeElement.textContent).toContain("Unable to load stats");
	});
